...
Connect to Dash0 to query data using Prometheus API and react to alert events
...
<CardGrid>
  ...
  <LinkCard title="On Notification" href="#on-notification" description="Trigger a workflow when Dash0 sends a notification" />
</CardGrid>
...
To set up the Dash0 integration:
...
The On Notification trigger starts a workflow execution when Dash0 sends a notification to SuperPlane.
